...
`-1 1 -> -43758.5453 to 43758.5453`

In our hash function we only want the decimal part of a number as the result. For that we use `fract()`. If our numbers inside it are small the decimal numbers might show some pattern. Multiply by huge numbers mean even tiny difference in sine value result in huge differences in the resulting integer part of the number, scrambling the decimal part into a pseudo random number.
```
...
1. Combine x and y into one number
